...
class PortfolioCoinDetailedView extends StatefulWidget {
  final String cryptoId;
  final double amount;
  const PortfolioCoinDetailedView(
      {super.key, required this.cryptoId, required this.amount});

  @override
  State<PortfolioCoinDetailedView> createState() =>
      _PortfolioCoinDetailedViewState();
}

class _PortfolioCoinDetailedViewState extends State<PortfolioCoinDetailedView> {
  ...
  Widget build(BuildContext context) {
    double finalPrice = widget.amount * (crypto.price ?? 0);
    ...
    for (var i = 0; i < pricesHistory.length; i++) {
      if (pricesHistory[i].price != null) {
        pricesHistoryChartData.add(FlSpot(
            ...
            pricesHistory[i].price! * (widget.amount)));
      }
    }
    setState(() {});
  }
